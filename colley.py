...
def colley(games, teams, opts={
    ...
    colley_matrix = 2 * np.diag(np.ones(num_teams))
    b = np.ones(num_teams)
    k = 10
    ...
    for i in range(num_games):
        ...
        colley_matrix[team_1_index, team_1_index] += game_weight
        colley_matrix[team_2_index, team_2_index] += game_weight
        colley_matrix[team_1_index, team_2_index] -= game_weight
        colley_matrix[team_2_index, team_1_index] -= game_weight

        # Update the b vector
        if team_1_score > team_2_score:
            b[team_1_index] += game_weight/2
            b[team_2_index] -= game_weight/2
        else:
            b[team_1_index] -= game_weight/2
            b[team_2_index] += game_weight/2
    ...
    colley_ratings = pd.DataFrame(columns=['rank', 'team', 'rating'])
    for i in range(num_teams):
        colley_ratings.loc[i, :] = [
